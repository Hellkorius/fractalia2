...
namespace Rendering {
    RenderingService& getService() {
        return ServiceLocator::instance().requireService<RenderingService>();
    }
    
    void registerEntity(flecs::entity entity) {
        getService().registerRenderableEntity(entity);
    }
    
    void unregisterEntity(flecs::entity entity) {
        getService().unregisterRenderableEntity(entity);
    }
    
    bool isVisible(flecs::entity entity) {
        // This would check the render queue for the entity's visibility
        // For now, return true if entity has required components
        return entity.has<Transform>() && entity.has<Renderable>() && 
               entity.get<Renderable>()->visible;
    }
    ...
    int getLODLevel(flecs::entity entity) {
        ...
        return getService().calculateLOD(transform->position, cameraPos);
    }
    ...
    float getDistanceToCamera(flecs::entity entity) {
        ...
        const Transform* transform = entity.get<Transform>();
        return glm::length(transform->position - cameraPos);
    }
    
    void setLODEnabled(bool enabled) {
        getService().setLODEnabled(enabled);
    }
    
    void setFrustumCullingEnabled(bool enabled) {
        getService().setFrustumCullingEnabled(enabled);
    }
    
    void setDebugMode(bool enabled) {
        getService().setDebugVisualization(enabled);
    }
    
    uint32_t getVisibleEntityCount() {
        return getService().getCullingStats().visibleEntities;
    }
    
    float getCullingRatio() {
        return getService().getCullingStats().getCullingRatio();
    }
    
    uint32_t getDrawCallCount() {
        return getService().getRenderStats().totalDrawCalls;
    }
    
    float getRenderTime() {
        return getService().getRenderStats().cpuRenderTimeMs;
    }
}