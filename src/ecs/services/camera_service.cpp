...
namespace CameraSystem {
    ...
    glm::vec2 worldToScreen(const glm::vec3& worldPos) {
        return getService().worldToScreen(worldPos, glm::vec2(800, 600));
    }
    
    glm::vec2 screenToWorld(const glm::vec2& screenPos) {
        return getService().screenToWorld(screenPos, glm::vec2(800, 600));
    }
    ...
    void moveCamera(const glm::vec3& delta) {
        getService().moveCamera(getService().getActiveCameraID(), delta);
    }
    
    void setCameraPosition(const glm::vec3& position) {
        getService().setCameraPosition(getService().getActiveCameraID(), position);
    }
    
    void setCameraZoom(float zoom) {
        getService().setCameraZoom(getService().getActiveCameraID(), zoom);
    }
    
    void focusOn(const glm::vec3& target, float zoom) {
        getService().focusCameraOn(getService().getActiveCameraID(), target, zoom);
    }
    
    void transitionTo(const glm::vec3& position, float zoom, float duration) {
        Camera targetState;
        targetState.setPosition(position);
        targetState.setZoom(zoom);
        getService().transitionActiveCameraTo(targetState, CameraTransitionType::LINEAR, duration);
    }
    
    void transitionTo(CameraID cameraID, float duration) {
        getService().transitionToCamera(cameraID, CameraTransitionType::LINEAR, duration);
    }
}